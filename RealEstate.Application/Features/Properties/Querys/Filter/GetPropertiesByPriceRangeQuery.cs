...
{
    public class GetPropertiesByPriceRangeQuery : IRequest<AppResponse<PaginationResponse<PropertyDTO>>>
    {
        public int? MinPrice { get; set; }
        public int? MaxPrice { get; set; }
        public PaginationRequest Pagination { get; set; }

        public GetPropertiesByPriceRangeQuery(PaginationRequest pagination, int? minPrice, int? maxPrice)
        {
    ...
    public class GetPropertiesByPriceRangeQueryHndler : IRequestHandler<GetPropertiesByPriceRangeQuery, AppResponse<PaginationResponse<PropertyDTO>>>
    {
        private readonly IPropertyRepository _propertyRepository; 
        private readonly IMapper _mapper;
        private readonly IFileManager _fileManager;
        public GetPropertiesByPriceRangeQueryHndler(
        ...
        public async Task<AppResponse<PaginationResponse<PropertyDTO>>> Handle(GetPropertiesByPriceRangeQuery request, CancellationToken cancellationToken)
        {

            Expression<Func<Property, bool>> filter = property =>
               (!request.MinPrice.HasValue || property.Price >= request.MinPrice.Value) &&
                (!request.MaxPrice.HasValue || property.Price <= request.MaxPrice.Value)
